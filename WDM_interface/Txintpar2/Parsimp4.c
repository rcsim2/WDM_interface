...
Abstract:
    ...
    //////////////////////////////////////////////////////////////
    AT LAST: IT WORKS: building it using the NT4 DDK and using
    RtlAppendUnicodeStringToString(LinkName, &linkDigits);
    which we had commented out to be able to use TXINTPAR instead
    of TXINTPAR1 in CreateFile() in Load.exe.
    ...
    Well, we can also build on Win2000 DDK and TXINTPAR or TXINTPAR1
    and RtlAppendUnicodeStringToString(LinkName, &linkDigits); make no
    difference: it still works: the ISR gets connected. Note that
...

{
    ...
    DbgPrint("We're in ParInterruptService\n");
...

{
    ...
    DbgPrint("We're in ParCompleteRequest\n");
