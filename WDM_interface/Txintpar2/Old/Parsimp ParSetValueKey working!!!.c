...

{
    ...
    //              (PWSTR)L"EnableConnectInterruptIoctlQQQ", 
    ...
    )
{
	...
	// Enum\Root\*PNP0400\PnPBIOS_10
	// NOTE: NO!!! this opens HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\
	// Enum\Root\*PNP0400\PnPBIOS_10\Device Parameters
    status = IoOpenDeviceRegistryKey(pdo, PLUGPLAY_REGKEY_DEVICE, KEY_ALL_ACCESS, &hPnPKey);    
    
    // Hell, we are getting STATUS_INVALID_DEVICE_REQUEST, meaning there is *possibly*
    // an invalid Pdo. But this can't be: we have tried calling IoOpenDeviceRegistryKey()
    // from a point where we were absolutely sure we had a valid Pdo (just after
    // IoGetDeviceObjectPointer()) and still we got STATUS_INVALID_DEVICE_REQUEST.
    // Maybe another driver is hogging the key???
    if( !NT_SUCCESS( status ) ) {
	...
	// Sod the docs: this is not necessary: we are already in
...
*/
    ...
    // Zw...Key routines that require an input KeyHandle.
    // NOTE: This replaces or creates a value entry for a key in the registry
    status = ZwSetValueKey( hPnPKey, &valueName, 0, REG_DWORD, ParameterValue, sizeof(ULONG) );
